    ...
    <div class="bg-white p-6 rounded-lg w-[90%] md:w-[60%] max-h-[90vh]">
      <div v-if="cartItems.length" class="mb-4">
        <h3 class="text-lg font-semibold mb-4 flex justify-between p-2">
          <span class="font-semibold">{{ t('your_cart') }}</span>
          <button class="flex items-center gap-2 bg-gray-300 px-3 py-1 hover:bg-gray-400 rounded">{{ t('payment') }} <img src="/images/payment-card-svgrepo-com.svg" :alt="t('checkbox')" class="w-5 h-5"></button>
        </h3>
              ...
              <tr>
                <th class="pl-4 w-8">
                  ...
                  <label for="checkbox"
                    ...
                    <img src="/images/check-box-svgrepo-com.svg" :alt="t('checkbox')" class="w-5 h-5">
                  </label>
                </th>
                <th class="p-4 text-left text-sm font-medium text-white">{{ t('image') }}</th>
                <th class="p-4 text-left text-sm font-medium text-white">{{ t('name') }}</th>
                <th class="p-4 text-left text-sm font-medium text-white">{{ t('unit_price') }}</th>
                <th class="p-4 text-left text-sm font-medium text-white">{{ t('amount') }}</th>
                <th class="p-4 text-left text-sm font-medium text-white">{{ t('price') }}</th>
                <th class="p-4 text-left text-sm font-medium text-white">{{ t('manipulation') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in cartItems" :key="item.product.name" class="even:bg-blue-50 mb-2">
                <td class="pl-4 w-8">
                  ...
                  <label :for="item.product.name"
                    class="relative flex items-center justify-center p-0.5 peer-checked:before:hidden before:block before:absolute before:w-full before:h-full before:bg-gray-50 w-5 h-5 cursor-pointer bg-blue-500 border border-gray-400 rounded overflow-hidden">
                    <img src="/images/check-box-svgrepo-com.svg" :alt="t('checkbox')">
                  </label>
                </td>
                <td class="p-4 cursor-pointer">
                  <img :src="item.product.image" :alt="item.product.image" class="w-12 h-12 object-cover rounded" />
                </td>
                ...
                <td class="p-4 text-sm">
                  <div class="flex justify-center items-center h-full hover:text-red-600">
                    <button @click="removeFromCart(item)">{{ t('delete') }}</button>
                  </div>
                </td>
              </tr>
            </tbody>
        ...
        <div class="border-t mt-2 pt-6 flex justify-between">
          <span class="text-xl font-semibold ">{{ t('total') }}:</span>
          <span class="text-xl font-bold text-green-600">{{ formatCurrency(totalPrice) }}</span>
        </div>
      </div>
      <div v-else class="mb-4">
        <p class="text-center text-gray-500">{{ t('your_cart_is_empty') }}</p>
      </div>
      <!-- Close Button -->
